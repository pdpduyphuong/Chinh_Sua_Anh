...
if uploaded_file is not None:
    ...
    with col2:
        ...
        if "processed_img" in st.session_state and os.path.exists(st.session_state["processed_img"]):
            ...
            # Đảm bảo Streamlit nhận diện đúng vị trí thư viện cục bộ
            ...
            # Thử nghiệm Import an toàn từ core
            try:
                from core import (
                    adjust_image_advanced,
                    rotate_or_flip_image,
                ...
                st.info(
                    "💡 **Cách xử lý:** Vui lòng kiểm tra lại file `requirements.txt` trên repository và đảm bảo đã khai báo đầy đủ dependencies.")
                st.stop()
            ...
            # -------------------------------------------------------------------
            # PHÂN TÍCH AI ĐỒNG BỘ THÔNG SỐ SLIDER
            # -------------------------------------------------------------------
            def analyze_image_ai(image_path):
                pil_img = Image.open(image_path).convert("RGB")
                img_np = np.array(pil_img)
            ...
            if uploaded_file is not None:
                ...
                with tab3:
                    ...
                    st.info(
                        "👉 **Hướng dẫn:** Click chuột trực tiếp vào vị trí trên bức ảnh bên dưới để chọn điểm chèn chữ.")
                ...
                with tab6:
                    ...
                    if crop_mode == "Tỉ lệ cố định (Aspect Ratio)":
                        ratio_option = st.selectbox(
                            "Chọn tỉ lệ cắt:",
                            ["1:1 (Vuông)", "4:3 (Chuẩn)", "16:9 (Màn hình rộng)", "3:4 (Chân dung)",
                             "9:16 (Story/Reels)"]
                        )
