...
def apply_filter(input_path: str, output_path: str, filter_type: str):
    """
    Áp dụng bộ lọc nghệ thuật lên ảnh.
    """
    ...
    if filter_type == "Trắng Đen (Grayscale)":
        ...
        np_img = np.array(img, dtype=np.float32)
        # Tăng kênh Red, giảm nhẹ Blue để tạo tone ấm vintage
        ...
        np_img = np.array(img, dtype=np.float32)
        # Tăng kênh Blue, giảm nhẹ Red để tạo tone lạnh
...
def add_text_to_image(input_path: str, output_path: str, text: str, position: tuple, font_name: str, font_size: int,
                      color: tuple):
